...
export const Hefe = ({ children: inputImageTexture }) => (
    <Node
        shader={shaders.Hefe}
        uniforms={{
            inputImageTexture,
            inputImageTexture2: resolveAssetSource(
                require(`${PATH}filters/edgeBurn.png`)
            ),
            inputImageTexture3: resolveAssetSource(
                require(`${PATH}filters/hefeMap.png`)
            ),
            inputImageTexture4: resolveAssetSource(
                require(`${PATH}filters/hefeGradientMap.png`)
            ),
            inputImageTexture5: resolveAssetSource(
                require(`${PATH}filters/hefeSoftLight.png`)
            ),
            inputImageTexture6: resolveAssetSource(
                require(`${PATH}filters/hefeMetal.png`)
            ),
        }}
    />
);
