...
class Chat extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      msgText: '',
      messages: [],
      firebaseRef: database().ref('Message'),
    };
  }
  ...
  getMessages() {
    ...
    const from_uid = this.props.auth.uid;
    const to_uid = uid;
    var chatRoomId = `${from_uid}-${to_uid}`;
    var reverseChatRoomId = `${to_uid}-${from_uid}`;

    this.state.firebaseRef.once('value').then(snapshot => {
      if (snapshot.exists()) {
        var key = Object.keys(snapshot.val());
        var filterKeyData = _.filter(key, (item, index) => {
          return chatRoomId === item || reverseChatRoomId === item;
        });
        if (filterKeyData.length > 0) {
          this.refOn(this.state.firebaseRef.child(filterKeyData[0]), message => {
  ...
  append = message => {
    ...
    this.state.firebaseRef.once('value').then(snapshot => {
      if (!snapshot.exists()) {
        this.state.firebaseRef.child(chatRoomId).push(message);
        this.refOn(this.state.firebaseRef.child(chatRoomId), message => {
          this.setState(previousState => ({
            messages: GiftedChat.append(previousState.messages, message),
          }));
        });
        ...
        if (filterKeyData.length > 0) {
          this.state.firebaseRef.child(filterKeyData[0]).push(message);
        } else {
          this.state.firebaseRef.child(chatRoomId).push(message);
          this.refOn(this.state.firebaseRef.child(chatRoomId), message => {
            this.setState(previousState => ({
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        <View style={{ alignItems: 'center', paddingVertical: wp('3%') }}>
          <StyledChatTitle>{'Messages'}</StyledChatTitle>
        </View>
