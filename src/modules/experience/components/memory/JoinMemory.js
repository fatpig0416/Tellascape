...
class JoinMemory extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  _fetchData = val => {
    this.props.navigation.setParams({ screenTitle: 'JoinMemory' });
    ...
    if (val === true) {
      if (
        ...
        let req = {
          ...
          uid: '',
          onSuccess: response => this.onSuccessGetMemory(response, routeType),
          onFail: (error, code) => {
            if (code === 500) {
              Alert.alert('Join Memory', error, [{ text: 'OK', onPress: () => this.onPressMoreModal('leave') }]);
            } else {
              Alert.alert('Join Memory', error, [{ text: 'OK', onPress: () => this.props.navigation.goBack() }]);
            }
          },
        };
  ...
  onSuccessGetMemory = (response, routeType) => {
    if (response && response.length > 0) {
      ...
      if (is_deleted === 1 || is_deleted === true) {
        ...
        Alert.alert(
          'Memory Ended',
          'Oops! This memory has been ended. No more media can be uploaded.',
          [
            {
              text: 'Okay',
              onPress: () => {
                ...
                this.props.setLocalMemory([]);
                this.props.navigation.replace('PostMemory', {
                  parentID: parentID,
                  childID: child_ID,
                });
              },
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      ...
      this.props.memory.isMemoryLoad &&
      this.props.memory_data &&
      this.props.memory_data.length > 0
    ) {
      const { is_deleted, parentID, child_ID, founder_uid } = this.props.memory_data[0];
  ...
  renderZoomModel = () => {
    const { zoomInItem, paused, isBuffer, muted } = this.state;
    return (
      <RNModal visible={this.state.isZoomIn} transparent={true}>
  ...
  onVideoUploadRequest = async item => {
    ...
    RNFFmpeg.execute(`-i ${uri} -r 30 -c:v libx264 -crf 23 -s 720x1280 ${output}`).then(
      ...
      async response => {
        ...
        setTimeout(() => {
          this.props.uploadMemoryVideo(videoObj);
        }, 1);
      }
    );
  };
  ...
  onLikeEvent = async selectedCarouselData => {
    const { mediaButtonType, selectedFilter } = this.state;
    const { other_user_profile, child_ID } = this.props.memory_data[0];
  ...
  onPressMoreModal = value => {
    ...
    if (value === 'delete') {
      setTimeout(() => {
        Alert.alert(
          'Delete Memory',
          'Are you sure you would like to delete your memory ?',
          [
            {
              text: 'Cancel',
              onPress: () => console.log('Cancel Pressed'),
  ...
  render() {
    ...
    if (this.props.memory.isMemoryLoad && this.props.memory_data && this.props.memory_data.length > 0) {
      ...
      const { localMemory, isQuickMemoryStarted } = this.props.memory;
      ...
      // let autoFenceArea = area(autoFence);
      let newArray = [...localMemory];
      newArray = newArray.filter((item, index) => {
