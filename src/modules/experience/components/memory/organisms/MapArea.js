    ...
    if (location) {
      return (
        <Marker identifier={`pin-${id}`} key={id} coordinate={location} zIndex={index}>
          <MarkerImage
            source={
              index - 1 === this.props.activeIndex ? theme.images.MEDIA_MEMORY_ACTIVE : theme.images.MEDIA_INACTIVE
            }
  ...
  render() {
    ...
    if (this.props.parentID) {
      try {
        ...
        if (this.props.eventType === 'QuickMemory') {
          const eventData = this.props.memory.memory_data;
          if (eventData[0].fenceBuffer) {
            const geofence = eventData[0].fenceBuffer;
          ...
          if (this.props.eventType === 'Live' || this.props.eventType === 'Post') {
            ...
            const eventData = this.props.memory.memory_data.filter(exp => exp.parentID.search(EventPostId) >= 0);
            if (eventData[0].fenceJson) {
              const geofence = eventData[0].fenceJson;
              if (eventData[0].shape === 'polygon') {
                this.coordinates = geofence.coordinates[0].map(item => ({
      ...
      if (!_.isEmpty(this.coordinates)) {
        ...
        const latitudeDelta = Math.abs(center.latitude - bounds.minLat) * 4.6;
        const longitudeDelta = Math.abs(center.longitude - bounds.minLng) * 4.6;
      ...
      if (this.props.markerCoordinate !== undefined && this.props.memory.memory_data.length > 0) {
        if (this.props.eventType !== 'QuickMemory') {
          centerObj = {
            latitude: this.props.memory.memory_data[0].centerPoint.lat,
            longitude: this.props.memory.memory_data[0].centerPoint.lng,
          };
        }
        ...
        if (this.props.eventType === 'QuickMemory' && this.props.isQuickMemoryStarted === true) {
          centerObj = {
            latitude: this.props.memory.memory_data[0].centerPoint.lat,
            longitude: this.props.memory.memory_data[0].centerPoint.lng,
          };
        ...
        {!_.isEmpty(this.coordinates) ? (
          <React.Fragment>
            {this.props.eventType === 'View' ? (
              // this.state.mapViewSnapshot ? (
              //   <MapSnapshotImage source={{ uri: this.state.mapViewSnapshot }} />
              // ) : (
              <>
                <MapView
                      ...
                      features: [
                        {
                          ...
                          geometry: {
                            type: 'Polygon',
                            coordinates: this.props.memory.memory_data[0].fenceJson.coordinates,
                          },
                        },
                      ],
                    }}
              ...
              <>
                ...
                >
                        ...
                        features: [
                          {
                            ...
                            geometry: {
                              type: 'Polygon',
                              coordinates: this.props.memory.memory_data[0].fenceJson.coordinates,
                              // this.props.eventType === 'QuickMemory'
                              //   ? this.props.memory.memory_data[0].fenceBuffer.coordinates
                              //   : this.props.memory.memory_data[0].fenceJson.coordinates,
                            },
                          },
                        ],
                      }}
                        ...
                        features: [
                          {
                            ...
                            geometry: {
                              type: 'Polygon',
                              coordinates: this.props.fence.geometry.coordinates,
                            },
                          },
                        ],
                      }}
                  ...
                  {centerObj !== null && (
                    <Marker coordinate={centerObj} zIndex={0}>
                      <StyledMarkerImage source={theme.images.MEMORY_PIN_ICON} style={{ resizeMode: 'contain' }} />
                    </Marker>
                  )}
                ...
                <StyledCircleButtonWrapper top={circleWrapperTop}>
                  <CircleButton
                    ...
                    marginTop={this.props.eventType !== 'QuickMemory' ? 0 : 200}
                  ...
                  {this.props.eventType === 'Live' || this.props.eventType === 'QuickMemory' ? (
                    <CircleButton
                      iconColor={'#ffffff'}
