    ...
    if (location) {
      return (
        <Marker identifier={`pin-${id}`} key={id} coordinate={location} zIndex={index}>
          <MarkerImage
            source={
              index - 1 === this.props.activeIndex ? theme.images.MEDIA_EVENT_ACTIVE : theme.images.MEDIA_INACTIVE
            }
    ...
    if (this.props.parentID) {
      try {
        ...
        if (this.props.eventType === 'Live' || this.props.eventType === 'Post') {
          const eventData = this.props.experience.event_data;
          ...
          const eventData = this.props.experience.event_data.filter(exp => exp.parentID.search(EventPostId) >= 0);
          if (eventData[0].fenceJson) {
            const geofence = eventData[0].fenceJson;
            if (eventData[0].shape === 'polygon') {
              this.coordinates = geofence.coordinates[0].map(item => ({
      ...
      if (!_.isEmpty(this.coordinates)) {
        ...
        const latitudeDelta = Math.abs(center.latitude - bounds.minLat) * 6.5;
        const longitudeDelta = Math.abs(center.longitude - bounds.minLng) * 6.5;
      ...
      // if (this.props.markerCoordinate !== undefined) {
      ...
      if (this.props.markerCoordinate !== undefined && this.props.experience.event_data.length > 0) {
        centerObj = {
          latitude: this.props.experience.event_data[0].centerPoint.lat,
          longitude: this.props.experience.event_data[0].centerPoint.lng,
        };
                      ...
                      features: [
                        {
                          ...
                          geometry: {
                            type: 'Polygon',
                            coordinates: this.props.experience.event_data[0].fenceJson.coordinates,
                          },
                        },
                      ],
                    }}
                ...
                >
                  ...
                  {centerObj !== null && (
                    <Marker coordinate={centerObj} zIndex={0}>
                      <StyledMarkerImage source={theme.images.EVENT_PIN_ICON} style={{ resizeMode: 'contain' }} />
                    </Marker>
                  )}
