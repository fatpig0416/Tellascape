...
const DetailCard = props => {
  ...
  return (
    <StyledCard marginBottom={marginBottom}>
      <StyledTextField label={'Title'} value={title} onChangeText={onChangeTitle} experienceType={experienceType} />
      <StyledTextField
        label={'Description'}
        value={description}
...
const StyledTextField = props => (
  <TextField
    ...
    renderLeftAccessory={() => (
      <Text
        style={{ color: props.experienceType === 'event' ? theme.orange.text : theme.cyan.text, fontSize: wp('4%') }}
      >{`✷ `}</Text>
    )}
  />
);
