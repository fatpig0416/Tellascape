...
import { selectEventData, selectedLiveEventData } from '../../reducers/event';
...
class PostStation extends Component {
  constructor(props) {
    super(props);
  ...
  _fetchData = val => {
    this.props.navigation.setParams({ screenTitle: 'PostStation' });
    ...
    if (val === true) {
      if (
        ...
        let req = {
          ...
          onFail: error => {
            Alert.alert('Post Station', error, [{ text: 'OK', onPress: () => this.props.navigation.goBack() }]);
          },
        };
  ...
  onLikeEvent = async selectedCarouselData => {
    const { mediaButtonType, selectedFilter } = this.state;
    const { other_user_profile, child_ID, trending } = this.props.station_data[0];
