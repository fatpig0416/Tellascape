...
const StyledCard = styled.View`
  width: ${CARD_WIDTH};
  margin-top: ${wp('3.88%')};
  margin-bottom: ${props => (props.marginBottom === 0 ? 0 : wp('11.94%'))};
  border-radius: 15;
...
const DetailEditButton = props => {
  ...
  let borderColor = experienceType === 'memory' ? '#44D6BF' : '#669afd';
  return (
    <StyledDetailEditButton {...props} borderColor={borderColor}>
      ...
      <StyledPencilWrapper>
        <StyledGradientOverlay
          ...
          colors={experienceType === 'memory' ? gradients.BackgroundGreen : gradients.BackgroundBlue}
...
const DetailCard = props => {
  const { title, onChangeTitle, description, onChangeDescription, category, onToggleModal, experienceType } = props;
  return (
    <StyledCard>
      <StyledTextField label={'Title'} value={title} onChangeText={onChangeTitle} />
      <StyledTextField label={'Description'} value={description} onChangeText={onChangeDescription} />
      <DetailEditButton category={category} onPress={onToggleModal} experienceType={experienceType} />
    </StyledCard>
  );
};
const GuestDetailCard = props => {
  const { description, onChangeDescription, experienceType } = props;
  return (
    <StyledCard>
      <StyledTextField label={'Description'} value={description} onChangeText={onChangeDescription} />
    </StyledCard>
...
const PostCard = props => {
  ...
  return (
    <StyledCard marginBottom={marginBottom}>
      ...
      <StyledPostDescription>{'Share your Station with the World Right Now! '}</StyledPostDescription>
      <PostSwitch
        marginTop={20}
        inactiveText={'Live'}
        activeText={'After'}
...
const StyledTextField = props => (
  <TextField
    ...
    renderFullLine={true}
    renderLeftAccessory={() => <Text style={{ color: theme.blue.text, fontSize: wp('4%') }}>{`✷ `}</Text>}
  />
);
...
const MapSelectButton = props => (
  <StyledMapButton {...props}>
    <StyledMapSelectButtonText>
      <Text style={{ color: theme.blue.text }}>{`✷ `}</Text>
      {'Select a Location'}
    </StyledMapSelectButtonText>
  </StyledMapButton>
);
